...
namespace vk_uploader
{
    ...
    typedef boost::function<void ()> on_browser_destroy_callback_t;
    ...
    public:
        ...
        browser_dialog (const char *url,
            const on_navigate_complete_callback_t &p_navigate_callback,
            const on_browser_destroy_callback_t &p_destroy_callback)
            : m_inital_location (url), m_navigate_callback (p_navigate_callback), m_destroy_callback (p_destroy_callback) {}
    ...
    private:
        ...
        // dialog
        void on_init_dialog ();
        void on_destroy () { m_pos.RemoveWindow (*this); if (m_wb2) m_wb2.Release (); m_destroy_callback (); }
        void on_timer (UINT_PTR) { KillTimer (m_timer_id); m_navigate_callback (this); }
        ...
        on_navigate_complete_callback_t m_navigate_callback;
        on_browser_destroy_callback_t   m_destroy_callback;
        UINT_PTR                        m_timer_id;
    ...
    inline void open_browser_dialog (const char *url,
        const on_navigate_complete_callback_t &p_nav_callback,
        const on_browser_destroy_callback_t &p_dstr_callback)
    {
        new CWindowAutoLifetime<ImplementModelessTracking<browser_dialog>>(core_api::get_main_window (), url, p_nav_callback, p_dstr_callback);
    }
}
#endif